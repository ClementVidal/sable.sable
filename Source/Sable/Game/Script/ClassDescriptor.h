...
template< class ClassType, Bool allocateOnStack >
struct AllocateNewObject
{ 
	static Void Do( lua_State* L )
	{
		lua_remove(L, 1);   // use classname:new(), instead of classname.new()
		ClassType obj;
		CArgDescriptor<ClassType>::PushValue( L, obj );
	}
};

template< class ClassType>
struct AllocateNewObject< ClassType, FALSE >
{ 
	static Void Do( lua_State* L )
	{
		lua_remove(L, 1);   // use classname:new(), instead of classname.new()
...
template< class ClassType >
struct InstanceProxy<ClassType, TRUE>
{ 
	Void Free( )
	{
...
template< class ClassType >
struct InstanceProxy<ClassType, FALSE>
{	
	Void Free( )
	{
		Obj = NULL;
...
public:
	...
	typedef InstanceProxy<ClassType, StoragePolicy<ClassType>::Value > InstanceProxy;
	typedef ReferenceProxy< ClassType > ReferenceProxy;
	...
	int Register( lua_State* L ) const
	{
		...
		if( InstanciationPolicy<ClassType>::Value )
		{
	...
	{
		AllocateNewObject< ClassType, StoragePolicy<ClassType >::Value >::Do( L );
