...
CImplLinuxFileSystem::CImplLinuxFileSystem( CFileSystem& publicImpl ) :
    CImplStubFileSystem(publicImpl)
{
}

CImplLinuxFileSystem::~CImplLinuxFileSystem()
{
}

Bool CImplLinuxFileSystem::IsValidFindData( const Void* data ) const
{
...
CFilePath CImplLinuxFileSystem::ProcessFindData( String dir, const Void* data  ) const
{
	/*
    const WIN32_FIND_DATA& findData = *(const WIN32_FIND_DATA*)data;
...
Bool CImplLinuxFileSystem::ListDirectory( const CFilePath& dir, CArray<CFilePath>& list ) const
{
